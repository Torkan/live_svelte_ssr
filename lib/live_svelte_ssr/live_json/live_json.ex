defmodule LiveSvelteSsr.LiveJson do
  ...
  def push_patch(socket, doc_name, new_data) do
    doc_atom = safer_string_to_atom("lj" <> doc_name)
    old_data = Map.get(socket.assigns, doc_atom)

    data_patch = Jsonpatch.diff(old_data, new_data)

    IO.inspect(data_patch, label: "JSONDIFF")

    # If there's no data in the patch, no reason to send it.
    if data_patch != %{} do
      socket
      |> Utils.assign(doc_atom, new_data)
      |> Utils.push_event("lj:patch", %{doc_name: doc_name, patch: data_patch})
    else
      socket
    end
  end
