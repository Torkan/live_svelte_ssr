defmodule LiveSvelteSsr.NodeJS.Supervisor do
  use Supervisor
  ...
  def call!(module, args \\ [], opts \\ []) do
    ...
    |> case do
      {:ok, result} -> result
      {:error, message} -> raise LiveSvelteSsr.NodeJS.Error, message: message
    end
  end
  ...
  def init(opts) do
    ...
    worker = Keyword.get(opts, :worker, LiveSvelteSsr.NodeJS.Worker)
